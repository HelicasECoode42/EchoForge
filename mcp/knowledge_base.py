...
class KnowledgeBase:
    ...
    def add_documents(self, documents: List[Dict[str, str]]) -> int:
        ...
        for doc in documents:
            ...
            document_id = doc.get("id") or hashlib.sha256(
                f"{title}:{content}".encode("utf-8", errors="ignore")
            ).hexdigest()[:20]
            document_version = str(doc.get("version", "1"))
    ...
    def _chunk_text(self, text: str, chunk_size: int = 500) -> List[str]:
        """Compatibility helper; production ingestion uses the configured chunker."""
        return [
            chunk.content
            for chunk in create_chunker("fixed_char", chunk_size=chunk_size).chunk(
                document_id="compat",
                title="",
                text=text,
            )
